...
#const CHUNK_SIZE = Vector3(64.0, 64.0, 64.0) # take side size into account when calculating
#
...
#
#func _ready():
	#pid = OS.create_process("res://chunk_executables/test_server", [])
	#await get_tree().create_timer(0.2).timeout
	#socket.connect_to_host("127.0.0.1", 9000)
...
#func request_generate(chunk_id: int, x: float, y: float, z: float) -> PackedVector3Array:
	...
	## send chunk_id and position
	#socket.put_32(chunk_id)
	#socket.put_float(x)
	#socket.put_float(y)
	#socket.put_float(z)
#
	## wait for vertex count
...
#
	...
	#while socket.get_available_bytes() < total_bytes:
		#await get_tree().process_frame
	#var bytes = socket.get_data(total_bytes)[1]
#
...
#func add_all_chunks() -> void:
	...
	#
				...
				#unload_chunk(id) # this will generate a minimalistic mesh, c will call free(null) but that's ok
				#
				#id += 1
#
#func load_chunk(chunk_id: int) -> void:
	...
	#
	#var raw_verticies: PackedVector3Array = await request_generate(chunk_id, chunk_instance.position.x, chunk_instance.position.y, chunk_instance.position.z)
	#chunk_instance.mesh_instance.mesh = build_mesh(raw_verticies)
#
